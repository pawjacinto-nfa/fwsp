<section class="workspace-section tech-support-page">
    ...
    <section class="panel support-ticket-panel">
        ...
        <?php foreach ($tickets as $ticket): ?><tr><td>#<?= e($ticket['id']) ?> <?= e($ticket['title']) ?><small class="d-block text-muted"><?= nl2br(e($ticket['description'])) ?></small></td><td><?= e($ticket['reporter_name'] ?: 'Anonymous') ?><small class="d-block text-muted"><?= e(str_replace("\n", ' / ', \App\Models\User::locationLabel($ticket))) ?></small></td><td><?= e($ticket['category']) ?></td><td><?= e($ticket['status']) ?></td><td><?= e($ticket['updated_label'] ?? '') ?></td></tr><?php endforeach; ?>
